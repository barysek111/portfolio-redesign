/**
 * Rokoko Brand-specific Figure — resolves image paths via /rokoko-brand/ asset helper.
 */
import type { ReactNode } from "react";
import { cn } from "@/lib/utils";
import { asset, figureAlt } from "./utils";
import { blockTitle, contentBlockLabel, body, screenTitle } from "./constants";
  ...
  if (callout && calloutFrame && !title && !caption) {
    return (
      <figure className={layout === "pair" ? "min-w-0 h-full" : "w-full"}>
        <FigureCaptionCallout media={image} mediaOnly copyBelowMedia={calloutCopyBelowMedia} />
      </figure>
    );
  }
